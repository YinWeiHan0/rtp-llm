// #include "internal_source/maga_transformer/devices/yitian_arm_impl/ArmCpuDevice.h"
                   ...
                   int step)
{
    parallel_for(batch_size, [&](int batch_id) {
        ...
        for (int i = 0; i < k; i++) {
            ...
            if (rand_num <= 0.0f || i == k - 1) { // æ¦‚çŽ‡æ›´æ–°
                ...
                break; // æ¦‚çŽ‡é€‰æ‹©ä¸€ä¸ªæˆ–æœ€åŽä¸€ä¸ªï¼Œç„¶åŽbreaké€€å‡º
            }
        }
...
void ArmCpuDevice::sampleGreedy(const GreedyParams& params) {
    ...
    *(tokens.dataWithOffset<int32_t>(step)) = id;
    ...
    unsigned one_seed = std::random_device{}(); // èŽ·å–ä¸€ä¸ªéšæœºç§å­
    for (size_t i = 0; i < batch_size; i++) {
        generator_lists[i] = std::mt19937(one_seed + i); // ä½¿ç”¨ä¸åŒçš„ç§å­åˆå§‹åŒ–æ¯ä¸ªç”Ÿæˆå™¨
    }
    if (random_seed) {
