...
void addQKVBias(void* qkv, const void* bias,
    ...
    const T* q_bias = (T*)bias;
    const T* k_bias = (T*)bias + num_heads * head_size;
    const T* v_bias = (T*)bias + (num_heads + kv_num_heads) * head_size;
    ...
    parallel_for(N, [&](int tid) {
        ...
        T* v_input = (T*)qkv + tid * (num_heads + 2 * kv_num_heads) * head_size + (num_heads + kv_num_heads) * head_size;
...
template<typename T>
void halfRope(Buffer& input, size_t step, size_t base, size_t embed_dim) {
...
void ArmCpuDevice::runOneBatch(const AttentionModuleParams& params, int batch, size_t seq_len, size_t step) {
    ...
    if (params.weights.qkv_weight->bias) {
        if (datatype == DataType::TYPE_FP32) {
            addQKVBias<float>(qkv, params.weights.qkv_weight->bias->data(), 1, seq_len, head_num, kv_head_num, size_per_head);
        } else if (datatype == DataType::TYPE_FP16) {
            addQKVBias<__fp16>(qkv, params.weights.qkv_weight->bias->data(), 1, seq_len, head_num, kv_head_num, size_per_head);
        } else {
    ...
    transposeDim12(std::move(k_in), *k_out);
    transposeDim12(std::move(v_in), *v_out);
    tEnd = std::chrono::steady_clock::now();
    diff = std::chrono::duration_cast<std::chrono::microseconds>(tEnd - tStart);
    logTime(diff, 3);
    ...
    if (step != 0) {
        ...
        softmax_qk_output = softmax({qk_output, *params.common.attention_mask, std::nullopt, scale});
    }
    tEnd = std::chrono::steady_clock::now();
    diff = std::chrono::duration_cast<std::chrono::microseconds>(tEnd - tStart);
    logTime(diff, 5);
    ...
    tStart = std::chrono::steady_clock::now();
    auto qkv_output = gemm({*softmax_qk_output, *v_out});
    tEnd = std::chrono::steady_clock::now();
    diff = std::chrono::duration_cast<std::chrono::microseconds>(tEnd - tStart);
    logTime(diff, 6);
...
template<typename T>
void updateKVCacheStride(const AttentionModuleParams& params, int batch, size_t seq_len, size_t step) {
    auto qkv           = params.input.data();
    auto head_num      = params.configs.head_num;
    auto kv_head_num   = params.configs.kv_head_num;
    auto size_per_head = params.configs.size_per_head;
    ...
    for (int i = 0; i < block_num; i++) {
        ...
        T* k_input = (T*)qkv + (i * block_tokens) * (head_num + 2 * kv_head_num) * size_per_head + head_num * size_per_head;
        T* v_input = (T*)qkv + (i * block_tokens) * (head_num + 2 * kv_head_num) * size_per_head + (head_num + kv_head_num) * size_per_head;
...
void ArmCpuDevice::runOneBatchStride(const AttentionModuleParams& params, int batch, size_t seq_len, size_t step) {
    auto datatype      = params.input.type();
    auto head_num      = params.configs.head_num;
    auto kv_head_num   = params.configs.kv_head_num;
    auto size_per_head = params.configs.size_per_head;
    ...
    tStart = std::chrono::steady_clock::now();
    if (params.common.kv_cache.has_value()) {
        updateKVCacheStride<float>(params, batch, seq_len, step);
    }
    tEnd = std::chrono::steady_clock::now();
    diff = std::chrono::duration_cast<std::chrono::microseconds>(tEnd - tStart);
    logTime(diff, 8);
    ...
    parallel_for(NN, [&](int tid) {
        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, seq_len, size_per_head, step + 1, 1.0,
            ...
            (float*)params.output.dataWithOffset(tid * size_per_head), head_num * size_per_head);
    });
    tEnd = std::chrono::steady_clock::now();
    diff = std::chrono::duration_cast<std::chrono::microseconds>(tEnd - tStart);
    logTime(diff, 6);
...
AttentionModuleOutput ArmCpuDevice::contextAttention(const AttentionModuleParams& params) {
    ...
    std::cout << "*** datatype: " << getDataTypeStr(datatype) << ", batch_sz: " << batch_size << ", seq_len: " << seq_len << std::endl;
    if (params.input.type() == DataType::TYPE_FP32) {
        for (int batch = 0; batch < batch_size; batch++) {
            ...
            runOneBatchStride(params, batch, context_len, 0);
        }
    } else if (params.input.type() == DataType::TYPE_FP16) {
        for (int batch = 0; batch < batch_size; batch++) {
            size_t context_len = *static_cast<int*>(params.common.input_lengths.dataWithOffset(batch));
            runOneBatch(params, batch, context_len, 0);
        }
    } else {
        throw OpException(OpErrorType::ERROR_UNIMPLEMENTED);
    }
...
AttentionModuleOutput ArmCpuDevice::decoderSelfAttention(const AttentionModuleParams& params) {
    ...
    if (params.input.type() == DataType::TYPE_FP32) {
        for (int batch = 0; batch < batch_size; batch++) {
            ...
            runOneBatchStride(params, batch, 1, step);
        }
    } else if (params.input.type() == DataType::TYPE_FP16) {
        for (int batch = 0; batch < batch_size; batch++) {
            size_t step = *static_cast<int*>(params.common.sequence_lengths.dataWithOffset(batch));
            runOneBatch(params, batch, 1, step);
        }
    } else {
        throw OpException(OpErrorType::ERROR_UNIMPLEMENTED);
    }
